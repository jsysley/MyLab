...
for(i in 1:T)
{
    CHD_left <- matrix(CHD[CHD[,1]<=Age[i],],ncol = 2)
    CHD_right <- matrix(CHD[CHD[,1]>Age[i],],ncol = 2)
    
    n[1,1] <- sum(CHD_left[,2]==1)
    n[1,2] <- sum(CHD_left[,2]==2)
    n[2,1] <- sum(CHD_right[,2]==1)
    n[2,2] <- sum(CHD_right[,2]==2)
    n_row <- rowSums(n)
    n_col <- colSums(n)
    n_sum <- sum(n)
